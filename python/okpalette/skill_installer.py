...
def skill_destination(agent: AgentName) -> Path:
    ...
    return base / "skills" / "okpalette" / "SKILL.md"


def load_packaged_skill() -> str:
    template = (
        resources.files("okpalette")
        .joinpath("skills", "okpalette", "SKILL.md")
        .read_text(encoding="utf-8")
    )
    return _render_packaged_skill(template, version=__version__)


def _render_packaged_skill(template: str, *, version: str) -> str:
    stamp = f"Generated by okpalette {version}.\n"
    lines = template.splitlines(keepends=True)
    if lines and lines[0].strip() == "---":
        for index, line in enumerate(lines[1:], start=1):
            if line.strip() == "---":
                insert_at = index + 1
                return "".join(lines[:insert_at]) + f"\n{stamp}" + "".join(lines[insert_at:])
